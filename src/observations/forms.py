...
class ObservingSessionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['datetime_start_ut'].help_text = 'Enter observing session start time in UTC. Current UTC time automatically added.'
        self.fields['datetime_end_ut'].help_text = 'Enter observing session end time in UTC. Can be estimated now or entered later when session ends.'
        
        # Add helpful text for the site name field
        self.fields['site_name'].help_text = 'The physical location where the observing session will take place.'


class SolarSystemForm(forms.ModelForm):
    class Meta:
        model = SolarSystem
        fields = ['session', 'antoniadi_scale', 'telescope_size_type', 'magnification_used', 
                 'eyepieces_used', 'filters_used', 'drawing', 'additional_notes', 
                 'celestial_body', 'altitude_degrees', 'central_meridian_deg', 
                 'phase_fraction', 'disk_diameter_arcsec']
        widgets = {
...
class StarForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['session', 'antoniadi_scale', 'telescope_size_type', 'magnification_used',
                 'eyepieces_used', 'filters_used', 'drawing', 'additional_notes', 
                 'star_name', 'magnitude_estimate', 'finder_chart_used']
        widgets = {
            'additional_notes': forms.Textarea(attrs={'rows': 4}),
        }
